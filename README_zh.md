...
- **轻量实现**：引擎使用 Rust，不依赖 GUI 工具包或 Python。
...
- **可选 LLM 润色**：只处理 ASR 最终稿，自动跳过结构化与敏感输入；接口失败或结果异常时
  回退原文，并支持 OpenAI-compatible 与 Anthropic 两类协议下的多家云端供应商。
- **本地识别日志**：按天保存结构化日志、最终识别文本、进入 LLM 润色前的 ASR 原文和
  IBus 应用上下文，不保存录音。
...
前端，也不支持 Fcitx5。项目不包含 Windows、macOS 客户端、选中文本改写、账号、云额度、
云端历史同步或词典系统。LLM 润色默认关闭，只有配置 `llm` 后启用；供应商与安全边界见
[LLM 润色文档](docs/llm/README.md)，本地日志范围见[日志说明](docs/logging.md)。

## 文档

从[完整文档站](https://day253.github.io/typeless-ibus/docs/)进入安装、使用、供应商配置、故障排查、架构和开发文档；
