...
src/llm/mod.rs                   润色策略、应用类别、提示词、回退与输出保护
src/llm/provider.rs              LLM trait、HTTP 重试、请求 ID 与通用响应边界
src/llm/openai_compatible.rs     品牌化云端供应商共享的 Chat Completions 协议
src/llm/anthropic.rs             Anthropic Messages 协议
...
最终结果没有 LLM 配置时直接使用 `CommitText` 写入当前应用。配置 LLM 后，ASR 最终稿仍
留在 preedit，润色成功并通过本地保护才替换，最后只提交一次；失败则提交 ASR 原文。

会话编号用于隔离过期异步结果：用户取消或开始新录音后，旧 ASR 或 LLM 任务不能再提交文字。

## LLM 供应商边界

LLM 是可选的最终稿处理器，不参与 ASR Partial 合并，也不读取周边文本。`LlmProvider`
输入只有本次语音文本、系统推断的语种、经过审核的应用类别与表达偏好。密码、PIN、终端、
数字、号码、URL、邮箱地址、姓名和未知用途在网络调用前确定性跳过。

绝大多数云端供应商共享 OpenAI Chat Completions compatible 适配器；Anthropic 使用独立
Messages 适配器。品牌 provider 只维护默认 endpoint/model/认证方式。输出必须通过语言、
长度、完成原因、包装格式和关键 token 保护；任一失败都回退到 ASR 原文，不需要第二个 LLM。
