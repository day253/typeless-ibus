...
物理按键 → IBus ProcessKeyEvent
        ...
        ALSA + cpal → 16 kHz PCM
             ...
             ASR Provider
              ┌───┴─────────────────┐
        豆包实时 WebSocket     OpenAI-compatible HTTP
          PCM → Opus              PCM → WAV
                  │
...
16-bit PCM 帧，再交给配置选中的 ASR provider。豆包 provider 边录边编码为 Opus 并发送，
可以用部分结果更新 IBus preedit；OpenAI-compatible provider 缓存当前录音、转换为 WAV，
在松开触发键后一次提交。最终结果使用 `CommitText` 写入当前应用。
...
豆包 WebSocket 建联后会读取响应头中的 `x-tt-logid`。OpenAI-compatible 接口会依次读取
`x-request-id`、`request-id` 或 `x-trace-id`。握手、传输或识别失败时，请求 ID 会随错误
写入日志，便于关联上游请求，同时不会记录 API Key。
