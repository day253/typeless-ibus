...
仓库里的 [`data/config.example.json`](../../data/config.example.json) 已预置所有通用字段。
首次配置时可以在仓库根目录直接复制，再手动修改 `provider` 和所选供应商需要的凭据：

```bash
install -Dm600 data/config.example.json ~/.config/typeless-ibus/config.json
```

未使用的字段保持 `null` 即可；`endpoint`、`model` 等 `null` 字段会继续使用对应供应商的
内置默认值。已有配置不要直接覆盖，请原地编辑或先自行备份。
