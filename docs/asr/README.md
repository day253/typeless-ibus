...
[文档首页](../index.md) · [使用与配置](../usage.md) · ASR 供应商
...
| 供应商 | `provider` | 配置说明 |
| --- | --- | --- |
| 豆包输入法相关服务（默认） | `doubao` | [零配置、自动凭据与实现来源](doubao.md) |
...
每个 `provider` 都提供“最小配置示例”和“最大配置示例”。除零配置的 `doubao` 外，最小
配置固定为 `provider + apiKey`；最大配置会列出该协议实现真正读取的全部可配置字段。

## 语种默认值

通常不需要在 JSON 中填写 `language`。引擎会参考系统 locale；如果系统是英文默认值但
时区位于中国，则优先使用中文。只有支持语言提示的 provider 才会收到该字段，不支持时
自动回退到模型检测。完整优先级、语言代码和 provider 能力见
...
仓库里的 [`data/config.example.json`](https://github.com/day253/typeless-ibus/blob/main/data/config.example.json) 是可以直接使用的最小
...
`endpoint`、`model`、`language`、`prompt` 和 `resourceId` 都有内置默认或可省略行为，
无需预先写入。只有需要覆盖默认值时才参考供应商文档的最大配置添加。已有配置不要直接
覆盖，请原地编辑或先自行备份。这些默认值由 Rust provider 实现在运行时解析，不依赖
文档或示例 JSON。
...
[返回 ASR 设计说明](../asr-providers.md) · [返回文档索引](../index.md)
