...
| Fedora | 43、44 | 对应版本的原生 `.rpm` | 原生构建、打包、安装、IBus 协议、卸载 |
| openSUSE | Tumbleweed | Tumbleweed 原生 `.rpm` | 原生构建、打包、安装、IBus 协议、卸载 |
...
Fedora 和 openSUSE 用户应选择与发行版及版本对应的 RPM，不能把某个 Fedora 版本构建的
RPM 当作通用 Linux 包安装。当前 RPM 首发架构为 x86_64，同时提供带有完整 vendored
Rust 依赖的 SRPM，便于其他 RPM 系发行版重建和适配。
