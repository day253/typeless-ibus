...
该命令不读取麦克风，也不依赖 IBus，并按配置文件选择 provider。默认豆包模式会在没有
本地凭据时自动获取凭据，然后检查 settings Token 接口以及 WebSocket 的 `StartTask`、
`StartSession` 握手，并隐藏所有凭据。首次握手失败时会使用同一设备身份重试；仍然失败
才注册一个仅用于诊断的临时身份，且不会覆盖本地凭据。

OpenAI-compatible 接口没有统一的无音频探测协议，因此该命令只校验并打印脱敏后的
endpoint、model 和鉴权状态；请用下面的真实音频诊断确认接口可用性。
...
文字。豆包失败日志中的 `x_tt_logid`，或 OpenAI-compatible 日志中的 `request_id`，可用于
排查对应的上游请求。
