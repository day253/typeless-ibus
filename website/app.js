const translations = {
  zh: {
    ...
    stepTwoBody: "默认是 Fn，也能改成左右 Ctrl、F8、F9、F10 或空格；“请说话…”会告诉你它已准备好。",
    stepThreeTitle: "文字跟着声音生长",
    stepThreeBody: "识别结果以原生 IBus 预编辑文本出现，长句和停顿也不会丢掉前文。",
    stepFourTitle: "松开，润色后写入",
    stepFourBody: "默认直接提交；配置 LLM 后先轻度润色，失败仍提交 ASR 原文，全程不碰剪贴板。",
    featuresEyebrow: "小而专注",
    featuresTitle: "只做好语音输入。",
    featuresIntro: "默认无需账号；需要时再接入自己的云端 ASR 与 LLM。润色只处理本次最终稿，异常自动回退原文。",
    nativeTitle: "原生 IBus",
    nativeBody: "通过发行版已有的 GTK、Qt、XIM 与 Wayland 模块覆盖日常应用。",
    rustTitle: "轻量 Rust 核心",
    rustBody: "不依赖 Python、GTK 设置程序或常驻网页服务，安装后安静运行。",
    directTitle: "直接提交文字",
    directBody: "不读取和覆盖剪贴板，不注入快捷键；LLM 关闭时文字不会发送到大模型。",
    providersEyebrow: "声音由你选择",
    providersTitle: "开箱即用，<br>也不被一家服务绑定。",
    providersBody: "默认豆包 ASR 无需配置；也能通过一个 JSON 选择自己的语音与润色服务。LLM 默认关闭，只处理安全的最终文本并随时回退原文。",
    providersGuide: "查看供应商与语种配置",
    llmGuide: "查看 LLM 润色配置",
  ...
  en: {
    ...
    manyProviders: "ASR + optional LLM",
    ...
    stepTwoBody: "Fn is the default, or choose left/right Ctrl, F8, F9, F10, or Space. “Please speak…” confirms it is ready.",
    ...
    stepFourTitle: "Release, polish, write",
    stepFourBody: "Commit directly by default, or opt into light LLM cleanup with automatic ASR fallback—never touching the clipboard.",
    featuresEyebrow: "Small and focused",
    featuresTitle: "Voice input. Done well.",
    featuresIntro: "Start without an account, then opt into your own cloud ASR and LLM when needed. Cleanup sees only the final transcript and always falls back safely.",
    ...
    directBody: "Never reads or replaces your clipboard and never injects shortcuts. With LLM off, text is never sent to a language model.",
    providersEyebrow: "Your voice, your provider",
    providersTitle: "Ready out of the box.<br>Never locked to one cloud.",
    providersBody: "Doubao ASR works with zero setup. One JSON file can also select your own speech and cleanup services. LLM stays off by default, sees only safe final text, and always falls back to ASR.",
    providersGuide: "Open provider and language guides",
    llmGuide: "Open LLM cleanup guide",
...
function renderStage(stage) {
  ...
  if (currentStage === 1) {
    ...
    const dots = document.createElement("span");
    dots.className = "preedit-ellipsis";
    dots.textContent = copy.listeningEllipsis;
    composePreedit.append(dots);
  }
